...
/**
 * Locale-Specific Pages Sitemap
 * 
 * Generates a sitemap for static pages in a specific locale
 * Includes hreflang annotations for all language versions
 * 
 * Route: /api/sitemap/pages/[locale]
 */
...
const PAGES: Page[] = [
    ...
    { slug: "login", priority: 0.5, changefreq: "monthly" },
    { slug: "register", priority: 0.5, changefreq: "monthly" },
    { slug: "cart", priority: 0.6, changefreq: "daily" },
    { slug: "my-account", priority: 0.6, changefreq: "weekly" },
    { slug: "dashboard", priority: 0.6, changefreq: "weekly" },
];
...
${PAGES.map((page) => {
    ...
    <priority>${page.priority}</priority>${LOCALES.map(
            (altLocale) => {
                const altUrl = page.slug ? `/${altLocale}/${page.slug}` : `/${altLocale}`;
                return `
