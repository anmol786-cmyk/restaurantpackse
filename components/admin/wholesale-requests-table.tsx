...
export function WholesaleRequestsTable({ initialRequests }: WholesaleRequestsTableProps) {
    ...
    const handleRejectConfirm = async () => {
        ...
        setIsRejecting(true);
        const result = await rejectWholesaleRequest(rejectTargetId, rejectionReason.trim());
        ...
        <>
            ...
            <Dialog open={rejectDialogOpen} onOpenChange={(open) => { if (!open) handleRejectCancel(); }}>
                <DialogContent className="sm:max-w-md">
                    <DialogHeader>
                        <DialogTitle>Reject Wholesale Application</DialogTitle>
                        <DialogDescription>
                            Please provide a reason for rejecting this application. This reason will be included in the notification email sent to the applicant.
                        </DialogDescription>
                    </DialogHeader>
                    <div className="py-2">
                        <Textarea
                            placeholder="Enter rejection reason (e.g. Incomplete business documentation, VAT number could not be verified, business type not eligible for wholesale...)"
                        ...
                        {rejectionReason.trim().length === 0 && (
                            <p className="mt-1.5 text-xs text-destructive">A rejection reason is required.</p>
                        )}
                    </div>
                    <DialogFooter className="gap-2">
                        <Button variant="outline" onClick={handleRejectCancel} disabled={isRejecting}>
                            Cancel
                        </Button>
                        <Button
                            variant="destructive"
