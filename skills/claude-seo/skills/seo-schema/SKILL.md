...
**Wholesale B2B specific (this project — restaurantpack.se):**
- `Wholesaler` — primary type for Anmol Wholesale (already implemented on homepage)
- `WholesaleStore` — use as @type alongside LocalBusiness for the physical location
- `OfferCatalog` — for product category pages listing bulk products
- `PriceSpecification` — for MOQ-based tiered pricing (minQuantity, maxQuantity, price)
- `QuantitativeValue` — for minimum order quantities on product offers
- `DeliveryChargeSpecification` — for shipping/delivery schema on product pages
- `B2BPurchasable` — flag wholesale-only products with `eligibleCustomerType: https://schema.org/Business`
